<?php
	...
	$sql_all = $mysqli->query("SELECT * FROM cd_show"); //mac dinh hien thi tat ca item
	...
	if(isset($_POST['filter'])){
		...
		if($name_cat =="Tất Cả"){ //kiem tra neu filter la "Tất cả" thì hiện thi tất cả tiem
			$sql_cat = $mysqli->query("select * from cd_show");
		}
		else{ // nguoc lai thi hien thi theo category
		$sql_cat = $mysqli->query("select * from cd_show WHERE category ='$name_cat'");
		}
	}
?>
				...
				<section class="catalog">
					...
					<?php if(isset($_POST['filter'])){ //chekc dieu kien neu chon filter -> hien ra ca cd theo category ca chon filter
						...
						echo' <span class="tag-sale"></span>';
							...
							<?php 
							if($obj_cat->sale >0){
								echo '<p class="price">'.(($obj_cat->price * (100- $obj_cat->sale))/100).' VNĐ <s>'.$obj_cat->price.' VNĐ</p>';
							} else {
						...
						echo '<span class="tag-sale"></span>';
							...
							<?php 
							if($obj_all->sale >0){
								echo '<p class="price">'.(($obj_all->price * (100- $obj_all->sale))/100).' VNĐ <s>'.$obj_all->price.' VNĐ</p>';
							} else {
